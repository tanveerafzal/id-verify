...
export const PartnerVerifications: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const checkAndRefresh = () => {
      if (detailsFetchedAt && Date.now() - detailsFetchedAt >= URL_REFRESH_INTERVAL) {
        console.log('[PartnerVerifications] Refreshing document URLs to prevent expiration');
  ...
  const handleSaveRetryCount = async () => {
    ...
    try {
      const response = await fetch(
        getApiUrl(`/api/partners/verifications/${selectedVerification.id}/retry-count`),
        {
          method: 'PUT',
          headers: {
            'Authorization': `Bearer ${token}`,
  ...
  const handleSaveDetails = async () => {
    ...
    try {
      const response = await fetch(
        getApiUrl(`/api/partners/verifications/${selectedVerification.id}/details`),
        {
          method: 'PUT',
          headers: {
            'Authorization': `Bearer ${token}`,
                  ...
                  <div className="retry-info">
                    {isEditingRetry ? (
                      ...
                      <div className="retry-display">
                        ...
                        {(selectedVerification.retryCount || 0) >= (selectedVerification.maxRetries || 5) && (
                          <span className="retry-exhausted">Max retries reached</span>
                        )}
                      </div>
                    )}
                  </div>
                        ...
                        <div className="checks-list-modal">
                          ...
                          {selectedVerification.results.checks.nameMatch !== undefined && (
                            <div className={`check-item-modal ${selectedVerification.results.checks.nameMatch ? 'pass' : 'fail'}`}>
                              ...
                              <span>
                                Name Match
                                {selectedVerification.results.checks.nameMatchScore && (
                                  <span className="face-score">
                                    ({Math.round(selectedVerification.results.checks.nameMatchScore * 100)}%)
                                  </span>
                                )}
                              </span>
                            </div>
